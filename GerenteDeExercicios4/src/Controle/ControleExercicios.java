...
public class ControleExercicios {
	...
	public void cadastraExercicios(String nomeProfessor, String nomeExercicio, List<Questao> exercicios){
		...
		if(m.containsKey(nomeProfessor)){
			...
			if(mapNomeprof.containsKey(nomeExercicio)){
				...
				if(listNomeExer != null){
					for(Questao q : listNomeExer){
						for(String s : q.getPergunta().split("\n")){
							if(s.equalsIgnoreCase(pergunta)){
	...
	public List<List<String>> getSorteaExercicio() throws ListaVaziaExeception{
		try {
			...
			for(int j = 0; j <listQuestao.size(); j++){
				pergunta.add(i, listQuestao.get(j).getPergunta());
	...
	public String getNotaExercicio(List<String> exer, List<String> resposta){
		...
		for(int i = 0; i < exer.size(); i++){
			if(exer.get(i).equals(resposta.get(i))){
				cont++;
			}
		}
		return "Quantidade de questões certas: " + cont;
	}
}